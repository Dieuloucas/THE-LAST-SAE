...
public class Controleur
{
	...
	private plateau.reseau.ServeurReseau 	serveurReseau;
	private plateau.reseau.ClientReseau   	clientReseau;
	private int                          	idJoueurReseauLocal;
	private plateau.ihm.PanelChargement 	panelChargement;
			...
			if (dir.exists() && dir.isDirectory())
			{
				...
				if (liste != null)
					for (int i = 0; i < liste.length; i++)
						if (liste[i].isFile() && liste[i].getName().endsWith(".txt")) txt.add(liste[i]);

				if (!txt.isEmpty())
				{
					File[] resultat = new File[txt.size()];
					for (int i = 0; i < txt.size(); i++) resultat[i] = txt.get(i);
					return resultat;
				}
			}
		}
	...
	// Permet au PanelChargement de se faire connaître par le Controleur
	public void setPanelChargement(plateau.ihm.PanelChargement panel) {
		this.panelChargement = panel;
	}
	...
	// Cache des images de cartes : imagesCartes[numéro][0 = claire, 1 = foncée]. Numéro 1..7 (7 = joker).
	private Image[][] imagesCartes = new Image[8][2];
	...
	public Image getImageCarte(Carte carte)
	{
		...
		int indice = carte.estFoncee() ? 1 : 0;
		if (numero < 1 || numero >= this.imagesCartes.length) return null;

		if (this.imagesCartes[numero][indice] == null)
		{
			String chemin = UtilitaireJeu.getCheminImageCarte(numero, carte.estFoncee());
			if (chemin != null) this.imagesCartes[numero][indice] = new ImageIcon(chemin).getImage();
		}
		return this.imagesCartes[numero][indice];
	}
	...
	private String chercherImage(String nom)
	{
		...
		{
			File f = new File(chemins[i]);
			if (f.exists()) return f.getAbsolutePath();
		}
		return "plateau/images/" + nom;
	}

	public void lancerServeurEtClient(int port, int nbManches, File fichierPlateau)
	{
		try 
		{
			this.chargerPlateau(fichierPlateau); // Charge le plateau en mémoire pour l'hôte
			this.serveurReseau = new plateau.reseau.ServeurReseau(port, this.metier, nbManches, fichierPlateau.getName());
			this.serveurReseau.start();
			Thread.sleep(100); 
			lancerClientUniquement("127.0.0.1", port);
		}
		catch (Exception e) {}
	}

	public void lancerClientUniquement(String ip, int port)
	{
		this.clientReseau = new plateau.reseau.ClientReseau(ip, port, this);
		this.clientReseau.connecter();
	}

	// Intercepteur réseau branché sur la méthode process() du SwingWorker
	public void traiterActionReseau(String message)
	{
		String[] segments = message.split(";");
		String ordre = segments[0];

		if (ordre.equals("ACCEPT_ID"))
		{
		...
		else if (ordre.equals("LOBBY_MAJ"))
		{
			...
			if (this.panelChargement != null && restants > 0)
			{
				if (this.serveurReseau != null) // Si je suis l'Hôte
				{
					String ipLocale = "127.0.0.1";
					try { ipLocale = java.net.InetAddress.getLocalHost().getHostAddress(); } catch (Exception e) {}
					
					this.panelChargement.setStatutLobby("Hôte [IP: " + ipLocale + " | Port: " + this.serveurReseau.getPort() + "] - " + restants + " joueur(s) restant(s).");
				}
				else // Si je suis l'Invité
				{
					this.panelChargement.setStatutLobby("Plateau choisi par l'hôte : " + nomPlateau + ". En attente de " + restants + " joueur(s)...");
				}
			}
		}
		else if (ordre.equals("START_GAME"))
		{
			...
			if (this.serveurReseau == null) 
			{
				...
				for (File f : dossiers) {
					if (f.getName().equals(nomPlateau)) {
						this.chargerPlateau(f);
						break;
					}
				}
			}

			// Ferme toutes les fenêtres d'accueil
			java.awt.Window[] windows = java.awt.Window.getWindows();
			for (java.awt.Window window : windows) {
				if (window instanceof plateau.ihm.FrameChargement || window instanceof plateau.ihm.FrameAccueil) {
					window.setVisible(false);
				}
			}

			this.lancerPartieLocale(nbManches);
		}
	}

	// Accesseur pour permettre aux boutons ou à la grille d'émettre vers les sockets
	public plateau.reseau.ClientReseau getClientReseau()
	{
		return this.clientReseau;
	}

	public int getIdJoueurReseauLocal()
	{
		return this.idJoueurReseauLocal;
	}
